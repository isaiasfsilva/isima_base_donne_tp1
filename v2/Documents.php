...
<div class="container">
     ...
     //SELECT GROUP_CONCAT(name SEPARATOR ' + ') AS order_summary FROM product WHERE product_id IN (13, 15, 16);
		$query = "SELECT Documents.id, Documents.anee, Documents.titre, Documents.type, Documents.niv_config, GROUP_CONCAT(Utilisateur.nom SEPARATOR ', ') auteurs FROM Documents LEFT JOIN Livres_auteurs ON Documents.id = Livres_auteurs.id_doc INNER JOIN Utilisateur ON Livres_auteurs.id_user = Utilisateur.id GROUP BY Documents.id";       
		$records_per_page=3;
